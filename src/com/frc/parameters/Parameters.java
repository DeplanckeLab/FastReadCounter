...
public class Parameters {
	...
	public static void load(String[] args) throws Exception {
				...
				switch (args[i]) {
					case "-o":
						...
						if (!f.exists()) {
							...
							new ErrorMessage(outputFolder + " is not a folder.");
					...
					case "-s":
						...
						switch (args[i]) {
							...
							default:
								new ErrorMessage(
										"The '-s' | '--stranded' option should be followed by one of the following parameters: [no, yes, reverse].");
						}
						break;
					case "--paired":
					case "-p":
					...
					case "--bam":
						...
						try {
							...
							if (!c.exists())
								new ErrorMessage("No file at path " + args[i]);
							if (!c.isFile())
								new ErrorMessage(args[i] + " is not a file");
							inputBAMFile = c;
						} catch (Exception e) {
							new ErrorMessage("The '--bam' option should be followed by aligned BAM file path. "
									+ e.getMessage() + ". You entered " + args[i]);
						}
						break;
					case "--gtf":
						i++;
						try {
							...
							new ErrorMessage("The '--gtf' option should be followed by GTF file path. " + e.getMessage()
									+ ". You entered " + args[i]);
						}
						break;
					case "--bamtag":
					...
					case "--umi-dedup":
						...
						switch (args[i]) {
							...
							default:
								new ErrorMessage(
										"The '--umi-dedup' option should be followed by one of the following parameters: [none, exact].");
						}
						break;
					case "--bed":
						i++;
						try {
							...
							new ErrorMessage("The '--bed' option should be followed by BED file path. " + e.getMessage()
									+ ". You entered " + args[i]);
						}
						break;
					case "--vcf":
						...
						try {
							...
							new ErrorMessage("The '-vcf' option should be followed by VCF file path. " + e.getMessage()
									+ ". You entered " + args[i]);
						}
						break;
					case "--barcodeFile":
						...
						try {
							...
							new ErrorMessage("The '--barcodeFile' option should be followed by a barcode file path. "
									+ e.getMessage() + ". You entered " + args[i]);
						}
						break;
					case "--barcodeTag":
					...
					case "-q":
						...
						try {
							...
							new ErrorMessage(
									"The '-q' option should be followed by an Integer. You entered " + args[i]);
						}
						break;
					case "-t":
					case "--threads":
						...
						try {
							...
							new ErrorMessage(
									"The '-t' | '--threads' option should be followed by an Integer. You entered "
											+ args[i]);
						}
						if (nbThreads < 1)
							new ErrorMessage(
									"The '-t' | '--threads' option should be followed by a positive Integer. You entered "
											+ nbThreads);
						int nbLogicalThreads = Utils.getNbLogicalThreads();
						if (nbThreads >= nbLogicalThreads)
							new WarningMessage("There are " + nbLogicalThreads
									+ " available logical threads on your machine. We would not recommend using more than "
									+ (nbLogicalThreads - 1) + " (here you've set " + nbThreads
									+ "). Optimal/maximum number should be ~nbLogicalThreads/2 = "
									+ nbLogicalThreads / 2);
						break;
		...
		if (inputGTFFile != null)
			c++;
		if (inputBEDFile != null)
			c++;
		if (inputVCFFile != null)
			c++;
		;
		if (c == 0)
			new ErrorMessage("Please use '--gtf', '--bed', or '--vcf' option to specify how to count the features");
		if (c > 1)
			new ErrorMessage(
					"You specified several features ('--gtf' '--bed' '--vcf'), while you should specify only ONE");
		if (inputVCFFile != null) {
		...
		if (!use_bam_tags) {
			if (Parameters.umi_dedup != UMIDedup.NONE)
				new ErrorMessage(
						"You cannot specify --umi-dedup option without the --bamtag option, i.e. FRC can only count UMIs that are already annotated in the BAM file.");
			System.out.println("Stranded = " + Parameters.stranded);
		} else {
			if (inputGTFFile == null)
				new ErrorMessage("The option --bamtag can only be used with option --gtf");
			System.out.println("Using BAM tags GN/GX for computing count matrix.");
			if (Parameters.umi_dedup != UMIDedup.NONE)
				System.out.println("UMI count matrix WILL BE generated using the " + Parameters.umi_dedup
						+ " deduplication scheme");
			else
				System.out.println("UMI count matrix WILL NOT BE generated (use '--umi-dedup' option to do so)");
		}
		...
		if (Parameters.doDemultiplexing) {
			...
			if (Parameters.barcodes.size() == 0)
				new ErrorMessage("\tNo barcodes were found in your barcode file.");
			System.out.println("\tBarcode file = " + Parameters.barcodes.size() + " barcodes of length "
					+ Parameters.barcodeLength);
		...
		if (outputFolder == null) {
			...
			new WarningMessage(
					"No output folder is specified, using the BAM folder as default. You can specify an output folder by using the '-o' option.");
			outputFolder = path;
		}
		outputFolder = outputFolder.replaceAll("\\\\", "/");
		if (!outputFolder.endsWith("/"))
		...
		if (nbThreads == 1)
			System.out.println("Run is NOT parallelized: 1 thread is used");
		else
			System.out.println("Run IS parallelized: " + nbThreads + " threads are used");
	}
	...
	public static void printHelp() {
		...
		System.out.println(
				"\t--bamtag \t\t[Use with --gtf] If BAM file is already mapped to genes (STARsolo) and/or tagged with GN/GX tags, use these tags to produce count matrix instead of the positions in the GTF file");
		System.out.println(
				"\t--barcodeFile %s \tPath of Barcode file for demultiplexing the BAM file using the barcode tag (default tag is 'CB')");
		System.out.println(
				"\t--barcodeTag %s \tChanging the barcode tag to check for the --barcodeFile option (default tag is 'CB')");
		System.out.println("\t--multiple-mapped %s \tKeep multiple mapped read (default = discard them)");
		System.out.println(
				"\t--umi-dedup %s \t\t[none, exact] How to dedup the UMIs (only if using --bamtag option, default = none)");
		System.out.println(
				"\t-s %s \t\t\t[no, yes, reverse] Do you want to count only reads falling on same strand than feature? (default = no, unused with option --bamtag)");
